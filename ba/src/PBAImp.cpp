		...
		for (i = 0; i < m_n3Dpts; i++)
		{
			...
			if (ptr != nullptr) {
				...
				*(ptr + 2) = *(ptr1 + 2) + xk[2];
				...
				throw std::runtime_error("空指针访问: ptr 不能为 NULL");
			}
		}
		...
		for(itno=0; itno<m_nMaxIter && !stop; ++itno)
		{
			...
			while(1) //determine increment using adaptive damping
			{
				...
				if(issolved)
				{
					...
					{
						if(eab_inf < (tmp=fabs(eab[i])))//eab[i]为参数向量第i分量的梯度项，雅可比矩阵转置*残差
							eab_inf=tmp;//梯度向量eab的无穷范数
						
						p_L2+=p[i]*p[i];//p[i]是参数向量的第i分量
					}								
					...
					for (i = 0, dp_L2 = 0.0; i < nvars; ++i)// compute p's new estimate and ||dp||^2 
					{
						...
						//		pdp[i - 1] = nangle[1];
						//		pdp[i - 2] = nangle[0];
						//		//printf("%f %f %f\n", pdp[i - 2], pdp[i - 1], pdp[i]);
						//		//pdp[i] = p[i] + dp[i];
						//		//pdp[i - 1] = p[i - 1] + dp[i - 1];
						//		//pdp[i - 2] = p[i - 2] + dp[i - 2];
						//		//printf("%f %f %f\n", pdp[i - 2], pdp[i - 1], pdp[i]);
						//	}
						//	else {
						//		pdp[i] = p[i] + dp[i];
						...
						tmp = dp[i];//改正量
						pdp[i]=p[i] + dp[i];
						dp_L2 += abs(tmp)*tmp;//改正量的平方和
					}

					//printf("%f %f %f %f %f\n", m_e1, mu, m_e3, eps1, eps3_sq);
					//dp_L2，参数变化量的L2范数的平方
					//p_L2，当前参数估计值的L2范数的平方
					//m_e1 = 0;收敛精度阈值（通常设为很小的正数，如1e-6）
					//如果本次迭代的参数变化量相对于当前估计值已经很小，则认为收敛
					//+m_e1是为了避免当前参数很小时判断失效
					
					if (sqrt(dp_L2) <= m_e1 * (sqrt(p_L2) + m_e1))//参数改正量小于阈值
					{
						stop = 1;	break;
					}
					...
					//评估当前步长是否有效，若ρ=dF/dL>0，实际误差下降比线性预期更好，则接受当前步长
					//否则回滚，增加mu
					//当mu很小时，近似高斯牛顿：步长大，速度快
					//当mu很大时，更像梯度下降：步长小，速度慢
					if((dF/dL)>0.0)
					{ 
	...
	void PBA::pba_jacobian(double *p, int* archor,struct sba_crsm *Uidxij, double*e,double* U,double* ea, double* V, double* eb, double *W, int n, int m, int numprojs, int *nphoto, int *nfeature )
	{
		...
		for ( i = 0; i < m_n3Dpts; i++ )
		{
			...
			for ( j = 0; j < numfea; j++ )
			{
				...
				//测试主锚点
				//double n1[2], n2[2];
				//pba_reprojectEachPts(m_KR, pa, ppt, nM, nN, nP, n1);
				//double sigma = 1e-6;
				//double* ppAM = pa + nM * 6 + 3;//指向主锚点
				//ppAM[0] += sigma;//主锚点的Xc+sigma
				//pba_reprojectEachPts(m_KR, pa, ppt, nM, nN, nP, n2);
				//double t1, t2;
				//t1 = (n2[0] - n1[0]) / sigma;
				//t2 = (n2[1] - n1[1]) / sigma;
				//printf("%f %f\n", t1, pAM[0 * 3 + 0]);
				...
				//double* ppt_new = ppt;
				//double* ppAM = pa + nM * 6 + 3;
				//double* ppAN = pa + nN * 6 + 3;
				...
				//	ba_updateKR(m_KR, m_KdA, m_KdB, m_KdG, m_K, pa);
				//	pba_reprojectEachPts(m_KR, pa, ppt, nM, nN, nP, n2);
				//	t1[gg] = (n2[0] - n1[0]) / sigma;
				//	t2[gg] = (n2[1] - n1[1]) / sigma;
				//	if (gg < 6)
				//		ppa_new[gg] -= sigma;
				//	else if (gg < 9)
				//		ppt_new[gg - 6] -= sigma;
				...
				//if (nP == nM)
				//{
				//	pPA[0 * 6] = t1[0];  pPA[0 * 6 + 1] = t1[1]; pPA[0 * 6 + 2] = t1[2]; pPA[0 * 6 + 3] = t1[3]; pPA[0 * 6 + 4] = t1[4]; pPA[0 * 6 + 5] = t1[5]; 
				//	pPA[1 * 6] = t2[0];  pPA[1 * 6 + 1] = t2[1]; pPA[1 * 6 + 2] = t2[2]; pPA[1 * 6 + 3] = t2[3]; pPA[1 * 6 + 4] = t2[4]; pPA[1 * 6 + 5] = t2[5];

				//	pPB[0 * 3] = t1[6]; pPB[0 * 3 + 1] = t1[7]; pPB[0 * 3 + 2] = t1[8];
				//	pPB[1 * 3] = t2[6]; pPB[1 * 3 + 1] = t2[7]; pPB[1 * 3 + 2] = t2[8];
				//	//printf("%f %f %f %f %f %f %f %f %f\n", t1[0], t1[1], t1[2], t1[3], t1[4], t1[5], t1[6], t1[7], t1[8]);
				//	//printf("%f %f %f %f %f %f %f %f %f\n", pPA[0 * 6], pPA[0 * 6 + 1], pPA[0 * 6 + 2], pPA[0 * 6 + 3], pPA[0 * 6 + 4], pPA[0 * 6 + 5], pPB[0 * 3], pPB[0 * 3 + 1], pPB[0 * 3 + 2]);
				//	//printf("%f %f %f %f %f %f %f %f %f\n", t2[0], t2[1], t2[2], t2[3], t2[4], t2[5], t2[6], t2[7], t2[8]);
				//	//printf("%f %f %f %f %f %f %f %f %f\n", pPA[1 * 6], pPA[1 * 6 + 1], pPA[1 * 6 + 2], pPA[1 * 6 + 3], pPA[1 * 6 + 4], pPA[1 * 6 + 5], pPB[1 * 3], pPB[1 * 3 + 1], pPB[1 * 3 + 2]);
				//	//printf("\n\n");
				//}
				//else if (nP == nN)
				//{
				...
				//	pAM[0 * 3] = t1[9]; pAM[0 * 3 + 1] = t1[10];  pAM[0 * 3 + 2] = t1[11];
				//	pAM[1 * 3] = t2[9]; pAM[1 * 3 + 1] = t2[10];  pAM[1 * 3 + 2] = t2[11];
				//	//printf("%f %f %f %f %f %f %f %f %f %f %f %f\n", t1[0], t1[1], t1[2], t1[3], t1[4], t1[5], t1[6], t1[7], t1[8], t1[9], t1[10], t1[11]);
				//	//printf("%f %f %f %f %f %f %f %f %f %f %f %f\n", pPA[0 * 6], pPA[0 * 6 + 1], pPA[0 * 6 + 2], pPA[0 * 6 + 3], pPA[0 * 6 + 4], pPA[0 * 6 + 5], pPB[0 * 3], pPB[0 * 3 + 1], pPB[0 * 3 + 2], pAM[0 * 3], pAM[0 * 3 + 1], pAM[0 * 3 + 2]);
				//	//printf("%f %f %f %f %f %f %f %f %f %f %f %f\n", t2[0], t2[1], t2[2], t2[3], t2[4], t2[5], t2[6], t2[7], t2[8], t2[9], t2[10], t2[11]);
				//	//printf("%f %f %f %f %f %f %f %f %f %f %f %f\n", pPA[1 * 6], pPA[1 * 6 + 1], pPA[1 * 6 + 2], pPA[1 * 6 + 3], pPA[1 * 6 + 4], pPA[1 * 6 + 5], pPB[1 * 3], pPB[1 * 3 + 1], pPB[1 * 3 + 2], pAM[1 * 3], pAM[1 * 3 + 1], pAM[1 * 3 + 2]);
				//	//printf("\n\n");
				//}
				//else
				//{
				...
				//	pAA[0 * 3] = t1[12]; pAA[0 * 3 + 1] = t1[13]; pAA[0 * 3 + 2] = t1[14];
				//	pAA[1 * 3] = t2[12]; pAA[1 * 3 + 1] = t2[13]; pAA[1 * 3 + 2] = t2[14];
				//	//printf("%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n", t1[0], t1[1], t1[2], t1[3], t1[4], t1[5], t1[6], t1[7], t1[8], t1[9], t1[10], t1[11], t1[12], t1[13], t1[14]);
				//	//printf("%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n", pPA[0 * 6], pPA[0 * 6 + 1], pPA[0 * 6 + 2], pPA[0 * 6 + 3], pPA[0 * 6 + 4], pPA[0 * 6 + 5], pPB[0 * 3], pPB[0 * 3 + 1], pPB[0 * 3 + 2], pAM[0 * 3], pAM[0 * 3 + 1], pAM[0 * 3 + 2], pAA[0 * 3], pAA[0 * 3 + 1], pAA[0 * 3 + 2]);
				//	//printf("%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n", t2[0], t2[1], t2[2], t2[3], t2[4], t2[5], t2[6], t2[7], t2[8], t2[9], t2[10], t2[11], t2[12], t2[13], t2[14]);
				//	//printf("%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n", pPA[1 * 6], pPA[1 * 6 + 1], pPA[1 * 6 + 2], pPA[1 * 6 + 3], pPA[1 * 6 + 4], pPA[1 * 6 + 5], pPB[1 * 3], pPB[1 * 3 + 1], pPB[1 * 3 + 2], pAM[1 * 3], pAM[1 * 3 + 1], pAM[1 * 3 + 2], pAA[1 * 3], pAA[1 * 3 + 1], pAA[1 * 3 + 2]);
				//	//printf("\n\n");
				//}
	...
	//printf("%f\n", m_e1);
		...
		{ // point
			...
			{ // view
				...
				// 	cost_function,
				// 	nullptr, // 使用默认 Huber loss 可加一个 HuberLoss(1.0) nullptr,new ceres::HuberLoss(1.0) loss_function
...
bool PBA::ba_initialize( char* szCamera, char* szFeature,  char* szCalib, char* szXYZ )
{
	...
	if (tpe == bal) {
		...
		{
			FILE* fpc = nullptr;
			fopen_s(&fpc, szCalib, "r");//打开内参文件
			nc_ = findNcameras(fpc) / 3;//读取相机数
			fclose(fpc);
			fpc = nullptr;
			// printf("%d\n", nc_);
			m_bFocal = false;
	...
	int** nfeature, int** archorSort)
{
	...
	*archorSort = (int*)malloc(*n3Dpts*3*sizeof(int));
	...
	ba_readCameraPose(fpc, *motstruct, m_V);//m_V指向了堆内存
	printf("读取到外参\n");
	// for (int i = 0;i < m_ncams; i++){
	...
	if (m_bProvideXYZ)
	{
		for (i = 0; i < m_n3Dpts; i++)
		{
			...
			double dispti2k;
			dispti2k = sqrt(pti2k[0] * pti2k[0] + pti2k[1] * pti2k[1] + pti2k[2] * pti2k[2]);//向量模长
			//主锚点到3D点的向量1
			...
			double dww = disDot2 / (dispti2k * sqrt(dW1));//主-副锚点向量与主-3D点向量的夹角
			...
			//注意：m_archor与m_archorSort不同，m_archor存的是主副锚点的实际编号，m_archorSort存的是主副锚点在当前track上的序号
			int id1 = cur + m_archorSort[i * 2];//3D点i在主锚点的uv坐标
			int id2 = cur + m_archorSort[i * 2 + 1];//3D点i在副锚点的uv坐标
